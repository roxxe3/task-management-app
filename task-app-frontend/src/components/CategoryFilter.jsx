...
const CategoryFilter = ({ categories, activeCategory, setActiveCategory, tasks }) => {
  const getCategoryTaskCount = (categoryName) => {
    ...
    return tasks.filter(task => task.category_name === categoryName).length;
  };
    ...
    <div className="mb-8 overflow-x-auto">
      <div className="flex space-x-4 pb-2">
        {categories.map((category) => {
          const taskCount = getCategoryTaskCount(category.category_name);
          return (
            <button
              ...
              title={`${category.category_name} (${taskCount} ${taskCount === 1 ? 'task' : 'tasks'})`}
            >
              <i className={`fas ${category.category_icon} ${activeCategory === category.category_name ? "text-black" : ""}`}></i>
              <span className="max-w-[150px] truncate">{category.category_name}</span>
